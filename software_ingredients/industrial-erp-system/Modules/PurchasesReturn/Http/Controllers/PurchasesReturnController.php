...
class PurchasesReturnController extends Controller
{
        ...
        DB::transaction(function () use ($request) {
            ...
            foreach (Cart::instance('purchase_return')->content() as $cart_item) {
                ...
                if ($request->status == 'Shipped' || $request->status == 'Completed') {
                    ...
                    $this->logStockChange('purchase_return', $purchase_return, $product, -$cart_item->qty);
                }
            }
        ...
        foreach ($purchase_return_details as $purchase_return_detail) {
            $cart->add([
                ...
                'options' => [
                    ...
                    'stock'       => Product::findOrFail($purchase_return_detail->product_id)->product_quantity + $purchase_return_detail->quantity,
        ...
        DB::transaction(function () use ($request, $purchase_return) {
            ...
            foreach ($purchase_return->purchaseReturnDetails as $purchase_return_detail) {
                if ($purchase_return->status == 'Shipped' || $purchase_return->status == 'Completed') {
                    ...
                    $this->logStockChange('purchase_return_update_reversal', $purchase_return, $product, $purchase_return_detail->quantity);
                }
            ...
            foreach (Cart::instance('purchase_return')->content() as $cart_item) {
                ...
                if ($request->status == 'Shipped' || $request->status == 'Completed') {
                    ...
                    $this->logStockChange('purchase_return_update', $purchase_return, $product, -$cart_item->qty);
                }
            }
    ...
    private function logStockChange(string $source, PurchaseReturn $purchaseReturn, Product $product, $delta): void
    {
