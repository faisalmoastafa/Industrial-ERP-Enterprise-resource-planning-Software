...
<div class="container">
    <div class="row mb-4 mt-2">
        <div class="col-12 d-flex justify-content-center">
            <img class="neci-brand-logo neci-brand-logo--auth" src="{{ settings()->getLogoSolidUrl() }}" alt="{{ settings()->company_name }}">
        </div>
    </div>
    <div class="row justify-content-center">
        <div class="col-md-6">
            <div class="card mx-4 border-0 shadow-sm yeti-auth-card">
                <div class="card-body p-4">
                    <form id="register" method="post" action="{{ url('/register') }}">
                        ...
                        <a href="{{ route('login') }}" class="text-center d-block">I already have a membership.</a>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    ...
    let caretPos, curEmailIndex, screenCenter, svgCoords,
        ...
        dFromC, eyeDistH, mouthStatus = "small", activeTrackingInput = null;

    function getCoord(inputEl) {
        if (!inputEl) return;
        let carPos = inputEl.selectionEnd,
            div = document.createElement('div'),
            span = document.createElement('span'),
            copyStyle = getComputedStyle(inputEl),
        ...
        emailCoords = getPosition(inputEl);
    ...
    function onTrackingInput(e) {
        activeTrackingInput = e.target;
        getCoord(activeTrackingInput);
    ...
    function onTrackingFocus(e) {
        activeTrackingInput = e.target;
        getCoord(activeTrackingInput);
    }

    function onTrackingBlur(e) {
        activeTrackingInput = null;
        resetFace();
    }
    ...
    [nameInput, email].forEach(input => {
        input.addEventListener('focus', onTrackingFocus);
        input.addEventListener('blur', onTrackingBlur);
        input.addEventListener('input', onTrackingInput);
    });

    [password, passwordConf].forEach(input => {
